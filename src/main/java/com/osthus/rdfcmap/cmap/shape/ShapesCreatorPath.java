...
/**
 * ShapesCreator based on AFO2.0 for star-shapes shapes
 * 
 * @author Helge Krieg, OSTHUS GmbH
 */
public class ShapesCreatorPath
{
	private static final Logger log = LogManager.getLogger("Logger");

	LinkedList<Resource> propertyPath = new LinkedList<Resource>();
	LinkedList<Boolean> directionPath = new LinkedList<Boolean>();
	Resource rootNodeShape;
	...
	{
		log.info("Creating concentric shapes based on property paths from TTL: " + pathToInputFile.toString()
		...
		rootNodeShape = shapesModel.createResource(AFOUtil.AFS_PREFIX + rootShapeTitle);
		rootNodeShape.addProperty(AFOUtil.RDF_TYPE, AFOUtil.SHACL_NODE_SHAPE);

		PathAndModel pathAndModel = new PathAndModel(pathList, shapesModel, rootNodeShape, 0);
		pathAndModel = findNeighbour(model, root, null, pathAndModel, true);

		rootNodeShape = pathAndModel.getNodeShape();
	...
	private Resource addTypeShapesBasedOnRdfSubClassOfStar(Resource resourceShape, Model model, Model shapesModel, Resource type)
	{
		...
		Resource blankNodeList = addResourcesAsRdfList(model, shapesModel, path);
	...
	private Resource addResourcesAsRdfList(Model fullModel, Model model, List<Resource> path)
	{
		Resource blankNodeList = model.createResource();
		...
		for (Iterator<Resource> iterator = path.iterator(); iterator.hasNext();)
		{
			...
			if (iterator.hasNext())
			{
				blankNodeRest = model.createResource();
			}
			else
			{
			...
			{
				List<Statement> blankNodeStatements = fullModel.listStatements(pathResource, (Property) null, (RDFNode) null).toList();
				Resource blankListMember = model.createResource();
	...
	private PathAndModel findNeighbour(Model model, Resource currentNode, Property link, PathAndModel pathAndModel, boolean isForwardLinked)
	{
		...
		while (nodeStmtIterator.hasNext())
		{
			...
			propertyPath.add(statement.getPredicate());
			directionPath.add(true);

			// now update shapesmodel for currentnode linking to nextnode
			Resource propertyShape = shapesModel.createResource();
			propertyShape.addProperty(AFOUtil.RDF_TYPE, AFOUtil.SHACL_PROPERTY_SHAPE);

			Resource propertyPathAsResource = getPropertyPath(propertyPath, directionPath, model);
			propertyShape.addProperty(AFOUtil.SHACL_PATH, propertyPathAsResource); // model.write(System.out,"TTL")
			...
			rootNodeShape.addProperty(AFOUtil.SHACL_PROPERTY, propertyShape);

			propertyPath.remove(propertyPath.getLast());
			directionPath.remove(directionPath.getLast());
		}
		// shapesModel.write(System.out,"TTL")
	...
	private Resource getPropertyPath(LinkedList<Resource> properties, LinkedList<Boolean> directions, Model model)
	{
		Resource pathList;
		if (!directions.contains(false) && properties.size() == 1)
		{
			pathList = properties.get(0);
		}
		else if (directions.contains(false) && properties.size() == 1)
		{
			pathList = shapesModel.createResource().addProperty(AFOUtil.SHACL_INVERSE_PATH, properties.get(0));
		}
		else if (!directions.contains(false) && properties.size() > 1)
		{
			pathList = addResourcesAsRdfList(model, shapesModel, properties);
		}
		else
		{
			pathList = createShaclPropertyPathWithInverseRelations(model, shapesModel, properties, directions);
		}

		return pathList;
	}

	private Resource createShaclPropertyPathWithInverseRelations(Model fullModel, Model model, LinkedList<Resource> path, LinkedList<Boolean> directions)
	{
		Resource blankNodeList = model.createResource();
		Resource blankNodeRest = null;
		int i = 0;
		for (Iterator<Resource> iterator = path.iterator(); iterator.hasNext();)
		{
			...
			if (!directionPath.get(i))
			{
				Resource inverseRelation = model.createResource();
				inverseRelation.addProperty(AFOUtil.SHACL_INVERSE_PATH, pathResource);
				member.addProperty(AFOUtil.RDF_FIRST, inverseRelation);
